...
if ($record_type === "Sleep") {
    ...
    if ($hours > 0 && $minutes > 0) {
        $details = "Slept for " . $hours . " hour" . ($hours > 1 ? "s" : "")
                 . " " . $minutes . " minute" . ($minutes > 1 ? "s" : "");
    } elseif ($hours > 0) {
        $details = "Slept for " . $hours . " hour" . ($hours > 1 ? "s" : "");
    } else {
        $details = "Slept for " . $minutes . " minute" . ($minutes > 1 ? "s" : "");
    }
...
elseif ($record_type === "Feeding") {
    ...
    // If solid food, get selected food ID
...
if ($stmt->execute()) {
    ...
    // =========================
    // SAVE SOLID FOOD IN BABY_MEALS
    // =========================

    if ($record_type === "Feeding" && $feeding_type === "Solid food") {

        $date_time = $record_date . " " . $time . ":00";

        $meal_notes = $_POST["additional_notes"] ?? "";

        $meal_stmt = $conn->prepare("
            INSERT INTO baby_meals
            (
                baby_id,
                food_id,
                meal_type,
